const { CURRENT_STEP, ROUTE_NUMBER } = require("../data/constants");

/**
 * ==========================================================
 * Accommodation Service
 * ==========================================================
 *
 * 역할
 * - 숙박 여행: 지역 기반 숙소 검색 링크 제공
 * - 여행 기간은 사용자에게 별도로 안내
 * - 당일치기: 숙소 단계 생략 후 출발지 수집
 * ==========================================================
 */

function buildYanoljaUrl(region) {
    return `https://www.yanolja.com/search/${encodeURIComponent(region)}`;
}

function handleAccommodationStep(facts) {

    if (facts.trip_type === "당일치기") {
        return {
            handled: true,
            facts,
            route_number: ROUTE_NUMBER.POST_BOOKING,
            current_step: CURRENT_STEP.ASK_START_LOCATION,
            last_question_field: "departure_location",
            reply:
                `${facts.region} 당일치기 여행이시군요. ` +
                `동선 추천을 위해 출발지를 입력해주세요.`,
        };
    }

    if (facts.trip_type === "숙박") {

        const yanoljaUrl =
            buildYanoljaUrl(facts.region);

        return {
            handled: true,
            facts,

            route_number:
                ROUTE_NUMBER.POST_BOOKING,

            current_step:
                CURRENT_STEP.ASK_ACCOMMODATION,

            last_question_field:
                "accommodation",

            reply:
                `${facts.region} 지역의 숙소를 확인해주세요.\n\n` +

                `여행 일정\n` +
                `체크인: ${facts.start_date}\n` +
                `체크아웃: ${facts.end_date}\n\n` +

                `야놀자 숙소 검색 링크:\n` +
                `${yanoljaUrl}\n\n` +

                `위 일정으로 숙소를 확인한 뒤, ` +
                `예약하신 숙소명이나 주소를 입력해주세요.`,
        };
    }

    return {
        handled: false,
        facts,
    };
}

module.exports = {
    buildYanoljaUrl,
    handleAccommodationStep,
};